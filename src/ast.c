...
ASTNode *ast_make_array_typed(const char*name,int size,int type){ASTNode*n=alloc_node(AST_ARRAY_TYPED);strncpy(n->var_name,name,MAX_LEXEME-1);n->var_name[MAX_LEXEME-1]='\0';n->num_value=size;n->op=(char)type;return n;}
...
ASTNode *ast_make_member(ASTNode*base,const char*member){ASTNode*n=alloc_node(AST_MEMBER);n->left=base;strncpy(n->var_name,member,MAX_LEXEME-1);n->var_name[MAX_LEXEME-1]='\0';n->num_value=0;n->op=0;return n;}
ASTNode *ast_make_call_indirect(ASTNode*fe,ASTNode*a){ASTNode*n=alloc_node(AST_CALL_INDIRECT);n->left=fe;n->right=a;return n;}
...
void ast_append_stmt(ASTNode *list,ASTNode*s){if(!s)return;if(!list->next){list->next=s;return;}ASTNode*t=list->next;while(t->next)t=t->next;t->next=s;}

void ast_free(ASTNode*n){if(!n)return;ast_free(n->left);ast_free(n->right);if(n->type==AST_PROGRAM||n->type==AST_BLOCK||n->type==AST_FUNC||n->type==AST_CASE||n->type==AST_DEFAULT||n->type==AST_CALL_INDIRECT){ASTNode*s=n->next;while(s){ASTNode*nx=s->next;s->next=NULL;ast_free(s);s=nx;}}free(n);}
