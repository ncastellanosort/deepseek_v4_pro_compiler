...
void ast_free(ASTNode*n){if(!n)return;ast_free(n->left);ast_free(n->right);if(n->type==AST_PROGRAM||n->type==AST_BLOCK||n->type==AST_FUNC){ASTNode*s=n->next;while(s){ASTNode*nx=s->next;s->next=NULL;ast_free(s);s=nx;}}free(n);}
